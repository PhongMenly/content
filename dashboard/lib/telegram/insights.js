...
const INSIGHTS_KEY = "bot_insights";
...
async function load() {
  return (await db.getKv(INSIGHTS_KEY)) || defaultInsights();
}

async function save(data) {
  await db.setKv(INSIGHTS_KEY, data);
}

// Log mỗi lượt hội thoại của khách (không phải owner)
async function logConversation(chatId, firstName, userMessage, botReply) {
